...
def predict(state:np.ndarray, std_p:float, std_v:float) -> np.ndarray:
    """
    Predicts the new position of the bird for the next frame basing on motion model.
    Params:
        state : NumPy array (4,M), the particles, rows are respectively x, y, v_x, v_y
...
def weight_particles(
    ...
    eta = state_bar[0:2, :] - z_extended  # innovation, (2,M)
...
def weight_particles_old(state_bar:np.ndarray, z:np.ndarray, std_q:float, threshold:float) -> np.ndarray:
    """
    Computes the weights of the particles.
    Params:
        state_bar : NumPy array (4,M), the particles, rows are respectively x, y, v_x, v_y
